(require '[cljs.build.api :as api]
         ...
         '[blockchain.core :refer [init]])

;;; Configuration.

(def source-dir   "src")
(def source-dirs  ["src/cljs"])


(def compiler-config {:main          "blockchain.core"
                      :output-to     "resources/js/blockchain.js"
                      :output-dir    "resources/js/out"
                      :asset-path    "js/out"
                      :optimizations :none
                      :parallel-build true})

(def dev-config (merge compiler-config {:optimizations :none}))
...
(defmethod task "figwheel" [[_]]
  (with-namespaces [figwheel-sidecar.repl-api]
    (figwheel-sidecar.repl-api/start-figwheel!
     {:figwheel-options {:nrepl-port       3312
